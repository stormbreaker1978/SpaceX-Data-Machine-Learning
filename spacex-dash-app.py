...
def scat(entered_site,payload):
    scafilt_df = spacex_df[spacex_df['Payload Mass (kg)'].between(payload[0],payload[1])]
    if entered_site == 'ALL':
        fig=px.scatter(
            scafilt_df,
            x='Payload Mass (kg)',
        ...
        selectedsite = scafilt_df[scafilt_df['Launch Site']==entered_site]
        fig=px.scatter(
            selectedsite,
            x='Payload Mass (kg)',
            y='class',
